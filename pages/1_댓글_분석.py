...
COMMUNICATION_COLUMNS = ["정보 질문", "개인 의사결정 질문", "경험 공유", "조언·오정보 가능성", "단순 반응"]
CLINICAL_COLUMNS = ["수술 후 증상·회복", "합병증·위험 불안", "자가관리 질문", "비용·접근성"]
AFFECTIVE_COLUMNS = ["정서 표현", "불확실성 표현", "신뢰 표지", "비표준 표기·반복·이모티콘"]
CATEGORY_COLUMNS = COMMUNICATION_COLUMNS + CLINICAL_COLUMNS + ["정서 표현"]

SHEET_HEADERS = [
    "원본 유튜브 링크", "댓글 원문", "좋아요 수", "작성일", "답글 수", "자동 제안 근거",
    "정보 질문", "개인 의사결정 질문", "경험 공유", "조언,오정보 가능성", "단순 반응",
    "수술 후 증상,회복", "합병증,위험 불안", "자가관리 질문", "비용,접근성",
    "의문문유형", "정서 표현", "불확실성 표현", "신뢰 표지", "비표준 표기,반복,이모티콘",
]
...
st.caption("열 순서: 의사소통 기능 → 임상적 주제 → 의문문 세부 유형 → 정서·언어 표지")

editable_columns = [
    "comment_key", "원본 유튜브 링크", "댓글 원문", "좋아요 수", "작성일", "답글 수", "자동 제안 근거",
    *COMMUNICATION_COLUMNS, *CLINICAL_COLUMNS, "의문문 유형", *AFFECTIVE_COLUMNS, "수기 확정",
]
...
if finalized.empty:
    ...
    summary = pd.DataFrame(
        {
            "범주": CATEGORY_COLUMNS,
            "댓글 수": [int(finalized[category].sum()) for category in CATEGORY_COLUMNS],
        }
    )
